...
class EnemyMeta:
    ...
    ID_ENEMIES: ClassVar[dict[str, "EnemyMeta"]] = {}
    KEY_ENEMIES: ClassVar[dict[str, "EnemyMeta"]] = {}
    ...
    def __post_init__(self):
        if self.prefer_quick:
            ...
            if self.quick_boss_meta.menu not in [
                I18nText.BossChallenge, I18nText.WeeklyChallenge, I18nText.NightmarePurification]:
                raise ValueError(f"Unsupported menu: {self.quick_boss_meta.menu}")
        else:
        ...
        if self.id in self.ID_ENEMIES:
            raise ValueError(f"Duplicate enemy id: {self.id}")

        # if not self.battle_text:
        #     raise ValueError("battle_text is empty")

        self.ID_ENEMIES[self.id] = self

        if self.key:
            if self.key.name in self.KEY_ENEMIES:
                raise ValueError(f"Duplicate enemy key: {self.key}")
            self.KEY_ENEMIES[self.key.name] = self
    ...
    def auto_respawn(self) -> bool:
        """是否自动刷新，部分梦魇boss"""
        if self.prefer_quick:
            return self.quick_boss_meta.auto_respawn
        return self.boss_meta.auto_respawn
    ...
    @property
    def absorb(self) -> Optional[AbsorbMode]:
        if self.prefer_quick:
            return self.quick_boss_meta.absorb
        return self.boss_meta.absorb

    def __eq__(self, other):
        if not isinstance(other, EnemyMeta):
            return False
...
class Enemy:
    ...
    NamelessExplorer = EnemyMeta(
        ...
        icon=EnemyIcon.Icon1,
        version=EnemyVersion.V3_1,
        sonata=[SonataEffect.SoundOfTrueName],
        elements=[EnemyElement.Aero],
        prefer_quick=True,
        boss_meta=BossMeta(
            name=_tr(I18nText.EnemyNamelessExplorer),
    ...
    NightmareAdamSmasher = EnemyMeta(
        ...
        cost=EnemyCost.Cost4,
        icon=EnemyIcon.Icon1,
        version=EnemyVersion.V3_4,
        sonata=[SonataEffect.ShadowOfShatteredDreams],
        elements=[],
        prefer_quick=True,
        boss_meta=BossMeta(
            name=_tr(I18nText.EnemyNightmareAdamSmasher),
